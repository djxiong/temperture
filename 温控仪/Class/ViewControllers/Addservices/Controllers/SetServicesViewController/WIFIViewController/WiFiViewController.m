...
- (void)setUI {
    ...
    xiaHuaXian.backgroundColor = [UIColor clearColor];
    [xiaHuaXian mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenH / 3, 1));
        make.centerX.mas_equalTo(self.view.mas_centerX);
    ...
    nameWiFi.textColor = kWhiteColor;
    ...
    self.ssidLabel.textColor = kWhiteColor;

    
    
    UIView *xiaHuaXian2 = [[UIView alloc]init];
    [self.view addSubview:xiaHuaXian2];
    xiaHuaXian2.backgroundColor = kWhiteColor;
    ...
    [self._pwdTextView setValue:kWhiteColor forKeyPath:@"_placeholderLabel.textColor"];
    self._pwdTextView.delegate = self;
    self._pwdTextView.secureTextEntry = YES;
    self._pwdTextView.keyboardType = UIKeyboardTypeDefault;
    
    self._confirmCancelBtn = [UIButton creatBtnWithTitle:@"搜索设备" andBorderColor:kMainColor WithTarget:self andDoneAtcion:@selector(tapConfirmCancelBtn:) andSuperView:self.view];
    self._confirmCancelBtn.backgroundColor = kCOLOR(239, 250, 253);
    self._confirmCancelBtn.layer.cornerRadius = kScreenW / 18;
    [self._confirmCancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenH / 3, kScreenW / 8));
        make.centerX.mas_equalTo(self.view.mas_centerX);
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    [UIView beginAnimations:@"ResizeForKeyboard" context:nil];
    [UIView setAnimationDuration:animationDuration];
    self.view.frame = CGRectMake(0, 0, kScreenW, self.view.frame.size.height);
    [UIView commitAnimations];
}
